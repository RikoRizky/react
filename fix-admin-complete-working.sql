-- Complete working admin management solution
-- This addresses all issues: RLS, UUID generation, and duplicate key constraints
...
DECLARE
    new_user_id UUID;
BEGIN
    -- Generate UUID first to avoid null constraint
...
BEGIN
    ...
    IF new_email IS NOT NULL AND new_email != (SELECT email FROM public.users WHERE id = target_user_id) THEN
