...
-- This completely bypasses RLS and uses direct table access

-- Disable RLS completely for admin operations
ALTER TABLE public.users DISABLE ROW LEVEL SECURITY;
ALTER TABLE auth.users DISABLE ROW LEVEL SECURITY;

-- Create function to add admin user with direct table access
...
-- Re-enable RLS for public.users only (keep auth.users disabled for admin operations)
ALTER TABLE public.users ENABLE ROW LEVEL SECURITY;

-- Create minimal RLS policies that allow admin operations
DROP POLICY IF EXISTS "Allow all operations for admins" ON public.users;
CREATE POLICY "Allow all operations for admins" ON public.users
